...
public class No811SubdomainVisitCount {
    public List<String> subdomainVisits(String[] cpdomains) {
        ...
        Map<String, Integer> timesOfDomain = new HashMap<>();

        for (String cpdomain : cpdomains) {
            String[] timesAndUrl = cpdomain.split(" ");
            int times = Integer.parseInt(timesAndUrl[0]);
            char[] url = timesAndUrl[1].toCharArray();

            for (int i = url.length - 1; i >= 0; i--) {
                if (url[i] == '.') {
                    String domain = new String(url, i + 1, url.length - i - 1);

                    if (!timesOfDomain.containsKey(domain)) {
                        timesOfDomain.put(domain, times);
                    } else {
                        timesOfDomain.put(domain, timesOfDomain.get(domain) + times);
                    }
                } else if (i == 0) {
                    String domain = new String(url, 0, url.length);
        ...
        for (Map.Entry<String, Integer> entry : timesOfDomain.entrySet()) {
     ...
     * "900 google.mail.com","5 wiki.org","5 org","1 intel.mail.com","951 com"]
     * 说明:
